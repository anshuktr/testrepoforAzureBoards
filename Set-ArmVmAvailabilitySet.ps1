...
##New features
#1.06
- Script modified by Brunoga based on the feedbak on TechnetGallery
#1.05
...
{
    $VMObject.StorageProfile.OsDisk.ManagedDisk.Id = (Get-AzureRMDisk -ResourceGroupName $VmRG -DiskName $($VMObject.StorageProfile.OsDisk.Name)).Id
...
for ($s=1;$s -le $VmObject.StorageProfile.DataDisks.Count ; $s++ )
{
    ...
    {
        $VmObject.StorageProfile.DataDisks[$s-1].ManagedDisk.Id = (Get-AzureRMDisk -ResourceGroupName $VmRG -DiskName $($VmObject.StorageProfile.DataDisks[$s-1].Name)).Id
